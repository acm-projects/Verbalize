...
export default function ClassroomHeader({ activePage }) {
  ...
  return (
    <>
      ...
      <div className={`${showModal ? 'blur-sm pointer-events-none' : ''} transition-all duration-300`}>
        <header className="relative text-white p-10 pt-16 bg-gradient-to-br from-[#3E6F8B] via-[#5087A9] to-[#2F3A4A]">
          <div className="w-full max-w-7xl pl-10 pr-4">
            <div className="flex items-center gap-6 mb-6 relative">
              ...
              <div>
                <div className="flex items-center gap-3 text-sm font-bold text-white/80 mb-1">
                  <span>CS 1200</span>
                  <span>Spring 2026</span>
                </div>
                <h1 className="text-3xl font-extrabold tracking-wide">Fundamental Programming</h1>
              </div>
            </div>

            {/* Navigation bar: Use activePage to determine which link should have the white underline */}
            <nav className="flex gap-8 text-lg font-bold mt-10">
              <Link href="/assignments" className={`pb-2 cursor-pointer transition-opacity border-b-4 ${activePage === 'assignments' ? 'border-white opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}>
                Assignments
              </Link>
              <Link href="/students" className={`pb-2 cursor-pointer transition-opacity border-b-4 ${activePage === 'students' ? 'border-white opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}>
                Students
              </Link>
              <Link href="/grades" className={`pb-2 cursor-pointer transition-opacity border-b-4 ${activePage === 'grades' ? 'border-white opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`}>
                Grades
              </Link>
              
              <button onClick={() => setShowModal(true)} className="border-b-4 border-transparent opacity-60 hover:opacity-100 pb-2 text-lg self-end mb-1 transition-opacity">
                + Add Assignments
              </button>
            </nav>
          </div>
